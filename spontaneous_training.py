...
def main(_):
    ...
    with strategy.scope():
        ...
        v1_rate_distribution_regularizer = losses.SpikeRateDistributionTarget(networks['v1'], spontaneous_fr=True, rate_cost=flags.rate_cost, pre_delay=delays[0], post_delay=delays[1], 
                                                                            data_dir=flags.data_dir, area='v1', core_mask=v1_core_mask, seed=flags.seed, dtype=dtype)
        lm_rate_distribution_regularizer = losses.SpikeRateDistributionTarget(networks['lm'], spontaneous_fr=True, rate_cost=flags.rate_cost, pre_delay=delays[0], post_delay=delays[1], 
        ...
        def roll_out(_x, _y, _w, output_spikes=False):
            ...
            v1_osi_loss = v1_OSI_Loss.original_version(_v1_z, _y)
            lm_osi_loss = lm_OSI_Loss.original_version(_lm_z, _y)
        ...
        delays = [int(a) for a in flags.delays.split(',') if a != '']
        # print(original_networks['v1']['synapses']['indices'])
        callbacks = Callbacks(model, optimizer, distributed_roll_out, flags, logdir, flag_str, strategy, 
                            ...
                            checkpoint=checkpoint, save_optimizer=False, spontaneous_fr=True)
...
if __name__ == '__main__':
    ...
    absl.app.flags.DEFINE_string('task_name', 'spontaneous_distr' , '')
    ...
    absl.app.flags.DEFINE_float('osi_cost', 0., '')
    ...
    absl.app.flags.DEFINE_boolean('train_interarea', False, '')
