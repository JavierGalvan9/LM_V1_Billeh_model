...
def main(_):
    ...
    with strategy.scope():
        ...
        # Option to resume the training from a checkpoint from a previous training session
            ...
            print(f'Restoring checkpoint from {flags.restore_from}...')
        ...
        v1_recurrent_regularizer = losses.StiffRegularizer(flags.recurrent_weight_regularization, networks['v1'], flags, penalize_relative_change=True)
        ...
        lm_recurrent_regularizer = losses.StiffRegularizer(flags.recurrent_weight_regularization, networks['lm'], flags, penalize_relative_change=True)
        ...
        v1_evoked_rate_regularizer = losses.SpikeRateDistributionTarget(networks['v1'], spontaneous_fr=flags.spontaneous_training, rate_cost=flags.rate_cost, pre_delay=delays[0], post_delay=delays[1], 
        ...
        lm_evoked_rate_regularizer = losses.SpikeRateDistributionTarget(networks['lm'], spontaneous_fr=flags.spontaneous_training, rate_cost=flags.rate_cost, pre_delay=delays[0], post_delay=delays[1], 
        ...
        # rate_loss = v1_evoked_rate_regularizer(rsnn_layer.output[0][0]) + lm_evoked_rate_regularizer(rsnn_layer.output[0][2])
        ...
        if flags.core_loss and v1_neurons > 51978:
            ...
            v1_annulus_rate_regularizer = losses.SpikeRateDistributionTarget(networks['v1'], spontaneous_fr=False, rate_cost=0.5*flags.rate_cost, pre_delay=delays[0], post_delay=delays[1],
            ...
            lm_annulus_rate_regularizer = losses.SpikeRateDistributionTarget(networks['lm'], spontaneous_fr=False, rate_cost=0.5*flags.rate_cost, pre_delay=delays[0], post_delay=delays[1],
        ...
        # model.add_loss(lambda: v1_OSI_DSI_Loss(rsnn_layer.output[0][0], placeholder_angle, trim=True, normalizer=v1_ema)[0] \
        #                     + lm_OSI_DSI_Loss(rsnn_layer.output[0][2], placeholder_angle, trim=True, normalizer=lm_ema)[0])
    ...
    def roll_out(_x, _y, _w, trim=True):
        ...
        osi_dsi_loss = v1_osi_dsi_loss[0] + lm_osi_dsi_loss[0]
        ...
        _aux = dict(rate_loss=rate_loss, voltage_loss=voltage_loss, regularizer_loss=regularizers_loss, osi_dsi_loss=osi_dsi_loss, sync_loss=sync_loss) #, sync_loss=sync_loss) #, weights_l2_regularizer=interarea_weights_l2_regularizer+recurrent_weights_regularizer)
        # _loss = osi_dsi_loss + rate_loss + voltage_loss + interarea_weights_l2_regularizer*100 + recurrent_weights_regularizer/100
    ...
    def train_step(_x, _y, _w, trim=True, output_metrics=False):
        ...
        with tf.control_dependencies([_op]):
            _op = train_sync_loss.update_state(_aux['sync_loss'])
        ...
        for g, v in zip(grad, model.trainable_variables):
            tf.print(f'{v.name}: ', 'Loss, average_gradient : ', _loss, tf.reduce_mean(tf.math.abs(g)))
            with tf.control_dependencies([_op]):
                _op = optimizer.apply_gradients([(g, v)])

        if output_metrics:
            return [0., _loss, _rate, _aux['rate_loss'], _aux['voltage_loss'], _aux['regularizer_loss'], _aux['osi_dsi_loss'], _aux['sync_loss']]
