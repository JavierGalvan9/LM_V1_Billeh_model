...
def main(_):
    ...
    with strategy.scope():
        ...
        v1_recurrent_regularizer = losses.StiffRegularizer(flags.recurrent_weight_regularization, networks['v1'], penalize_relative_change=True, dtype=tf.float16)
        ...
        lm_recurrent_regularizer = losses.StiffRegularizer(flags.recurrent_weight_regularization, networks['lm'], penalize_relative_change=True, dtype=tf.float16)
        ...
        v1_spont_rate_regularizer = losses.SpikeRateDistributionTarget(networks['v1'], spontaneous_fr=True, uniform_distribution_constraint=flags.spontaneous_uniform_distribution_constraint, rate_cost=flags.rate_cost, pre_delay=delays[0], post_delay=delays[1], 
                                                                            data_dir=flags.data_dir, area='v1', core_mask=v1_core_mask, seed=flags.seed, dtype=dtype)
        model.add_loss(lambda: v1_spont_rate_regularizer(rsnn_layer.output[0][0]))
        lm_spont_rate_regularizer = losses.SpikeRateDistributionTarget(networks['lm'], spontaneous_fr=True, uniform_distribution_constraint=flags.spontaneous_uniform_distribution_constraint, rate_cost=flags.rate_cost, pre_delay=delays[0], post_delay=delays[1], 
                                                                            data_dir=flags.data_dir, area='lm', core_mask=lm_core_mask, seed=flags.seed, dtype=dtype)
    ...
    def split_train_step(_x, _y, _w, _x_spontaneous, trim=True):
        ...
        with tf.control_dependencies([_op]):
            _op = train_regularizer_loss.update_state(_aux_gratings['regularizer_loss'] + _aux_spontaneous['regularizer_loss'])
        with tf.control_dependencies([_op]):
            _op = train_osi_dsi_loss.update_state(_aux_gratings['osi_dsi_loss'] + _aux_spontaneous['osi_dsi_loss'])
        with tf.control_dependencies([_op]):
