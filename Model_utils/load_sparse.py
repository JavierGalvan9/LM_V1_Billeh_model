...
def load_network(path,
  ...
  #                              3.33087865e-34, 1.26318969e-03, 1.20919572e-01])}
    ...
    if connected_selection: # this condition takes the n_neurons closest neurons 
        sorted_ind = np.argsort(r) #order according to radius distance. This option allows for more synapses since close neurons are more probably connected
...
def reduce_input_population(input_population, new_n_input, seed=3000):
    ...
    for input_neuron in range(input_population['n_inputs']): # go through all the asignment selection made 
        ...
        sel = in_ind[:, 1] == input_neuron  # consider neurons connected to the input_neuron
...
class InterareaConnectivity:
    def __init__(self, target_network, source_network, target_column_name,
        ...
        interarea_connectivity_path = os.path.join(self.data_dir, f'{self.source_column_name}_to_{self.target_column_name}_connection_probabilities.json')
    ...
    def assign_weight_and_delay(self, source, target, source_tf_ids, target_tf_ids, nsyns_ret):
        ...
        json_to_receptor_type = {'e2i.json':3, 'e2e.json':1, 'i2e.json':2, 'i2i.json':4}
    ...
    def connect_cells(self, source, target, params):
        ...
        sources_tuning_angle = np.repeat(sources_tuning_angle, num_neuron_target)
        ...
        if not np.isnan(gradient):
            ...
            delta_orientation = np.array(sources_tuning_angle, dtype=float) - np.array(targets_tuning_angle, dtype=float)
        ...
        nsyns_ret[nsyns_ret == 1] = self.rd.randint(nsyn_range[0], nsyn_range[1], len(nsyns_ret[nsyns_ret == 1]))
    ...
    def __call__(self):  
        ...
        for source in self.interarea_connectivity.keys():
            for target in self.interarea_connectivity[source].keys():
                ...
                if self.interarea_weight_distribution == 'levy_stable':
                    random_weights = levy_stable.rvs(0.6, 0, size=n_connections*10) # alpha, betas were got from fitting recurrent weights within a column
                    random_weights = random_weights[random_weights>0]
                elif self.interarea_weight_distribution == 'normal':
                    random_weights = self.rd.randn(n_connections*10) + 10 # mean and std were got from fitting recurrent weights within a column
                    random_weights = random_weights[random_weights>0]
        ...
        if target_tf_ids and source_tf_ids: # if both the lists are not empty
            ...
            r = np.concatenate(interarea_receptor_types) - 1  # Indentify the which of the 4 types of inputs we have
            indices = np.array([np.concatenate(target_tf_ids) + r, np.concatenate(source_tf_ids)], dtype=np.int64).T
            
            dense_shape = (self.target_network['n_nodes']*4, self.source_network['n_nodes']) #(all neurons in target column*n_receptors, all neurons in source column)
            ...
            indices, interarea_weights, dense_shape, interarea_delays = None, None, None, np.ones(1) # max_delay in model.py needs a non- None type 
