...
class OsiDsiCallbacks:
    ...
    def fanos_figure(self, spikes, area='', n_samples=100, spont_fano_duration=300, evoked_fano_duration=300, analyze_core_only=True, data_dir='Synchronization_data'):
        ...
        evoked_fanos, evoked_bin_sizes = self.fano_factor(spikes, area=area, t_start=evoked_t_start_seconds, t_end=evoked_t_end_seconds, n_samples=n_samples, analyze_core_only=analyze_core_only)

        spont_t_start_seconds = 0.2
        spont_t_end_seconds = spont_t_start_seconds + spont_fano_duration / 1000
        spontaneous_fanos, spont_bin_sizes = self.fano_factor(spikes, area=area, t_start=spont_t_start_seconds, t_end=spont_t_end_seconds, n_samples=n_samples, analyze_core_only=analyze_core_only)
        ...
        evoked_exp_data_path = os.path.join(data_dir, f'Fano_factor_{area}', f'{area}_fano_running_{evoked_fano_duration}ms_evoked.npy')
        evoked_exp_fanos = np.load(evoked_exp_data_path, allow_pickle=True)
        spont_exp_data_path = os.path.join(data_dir, f'Fano_factor_{area}', f'{area}_fano_running_{spont_fano_duration}ms_spont.npy')
        ...
        plt.savefig(os.path.join(self.images_dir, 'Fano_Factor', f'{area}_epoch_{self.current_epoch}_spont_{spont_fano_duration}ms_evoked_{evoked_fano_duration}ms.png'), dpi=300, transparent=False)
        plt.close()
    ...
    def osi_dsi_analysis(self, v1_spikes, lm_spikes, DG_angles, fano_duration=300):
        ...
        for axis_id, spikes, area in zip([0, 1], [v1_spikes, lm_spikes], ['v1', 'lm']):
            ...
            t_fano0 = time()
            self.fanos_figure(spikes, area=area, n_samples=1000, spont_fano_duration=300, evoked_fano_duration=300, analyze_core_only=True)
            self.fanos_figure(spikes, area=area, n_samples=1000, spont_fano_duration=300, evoked_fano_duration=1800, analyze_core_only=True)
