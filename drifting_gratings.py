...
def main(_):
    ...
    path = os.path.join(flags.save_dir, f'orien_{str(flags.gratings_orientation)}_freq_{str(flags.gratings_frequency)}_V1_{V1_neurons}_LM_{LM_neurons}')
    ...
    if LM_neurons > LM_core_neurons:
        # core_network = load_sparse.load_network(core_only=True)
        LM_core_mask = other_billeh_utils.isolate_core_neurons(networks['LM'], radius=LM_core_radius, data_dir=flags.data_dir)
    else:
    ...
    for stimuli in range(0, flags.n_simulations):
        ...
        out = extractor_model((inputs, dummy_zeros, state)) #inputs should go here
        ...
        V1_voltage = np.array(tf.cast(V1_voltage, tf.float16))[:, :, V1_core_mask]
        LM_voltage = np.array(tf.cast(LM_voltage, tf.float16))[:, :, LM_core_mask]
    ...
    graph = InputActivityFigure(networks, flags.data_dir, image_path, filename=raster_filename, frequency=flags.gratings_frequency)
...
if __name__ == '__main__':
    ...
    absl.app.flags.DEFINE_integer('LM_neurons', 80, '')  # -1 to take all neurons
    absl.app.flags.DEFINE_integer('n_input', 17400, '')  # number of LGN filters in visual space (input population)
