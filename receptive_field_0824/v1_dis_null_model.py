...
for neuron in range(n_neurons_v1):
    ...
    if len(above_std) > 0:
        ...
        # labelling disconnected components
        above_std_map = skimage.measure.label(above_std_map_binary)
        # get the largest disconnected component of each map
    ...
    if inverse:
        # obtain lists of the positions of all the disconnected components
        disconnected_components = label(below_std_map_binary)

        # Identify the largest disconnected component
        largest_cc_label = max(regionprops(disconnected_components), key=lambda r: r.area).label

        # Create a mask for the largest disconnected component and set it to 0
        below_std_map_binary[disconnected_components == largest_cc_label] = 0
...
# save the data
with open('Data_disconnected/null_chi2_v1_bootstrap.pkl', 'wb') as f:
    pkl.dump(null_chi2_v1, f)

with open('Data_disconnected/real_chi2_v1_bootstrap.pkl', 'wb') as f:
